@fees_heads.controller 'CreateStudentFeesController', [
  ...
  ($scope, Student, $timeout) ->
    ...
    $scope.$watch 'student.updatedAt', (time) ->
        return unless time?
        render()
    ...
    $scope.create = ->
      ...
      $scope.student.update().then (response) ->
        $scope.alert = true
        $scope.student.updatedAt = response.updatedAt
        $scope.student.studentsFeesHeads = response.studentsFeesHeads
        $timeout(render)

    # TODO: add generic class for filtering easyly by multiple fields in one time.
